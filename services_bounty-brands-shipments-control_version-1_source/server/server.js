...
// Proxy route for Freshdesk API calls (HTTP)
app.use('/api/freshdesk', async (req, res, next) => {
    // Handle OPTIONS request for CORS preflight
    if (req.method === 'OPTIONS') {
        res.setHeader('Access-Control-Allow-Origin', '*'); // Adjust as needed for security
        res.setHeader('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS');
    ...
    try {
        ...
        if (!freshdeskDomain) {
            console.error("Missing FRESHDESK_DOMAIN environment variable");
            return res.status(500).json({ error: "Server configuration error: Missing Freshdesk Domain" });
        }
        const freshdeskApiKey = process.env.FRESHDESK_API_KEY;
        if (!freshdeskApiKey) {
            console.error("Missing FRESHDESK_API_KEY environment variable");
            return res.status(500).json({ error: "Server configuration error: Missing Freshdesk API Key" });
        }

        const targetPath = req.url.startsWith('/') ? req.url.substring(1) : req.url;
        const apiUrl = `https://${freshdeskDomain}/${targetPath}`;
        console.log(`Freshdesk Proxy: Forwarding request to ${apiUrl}`);
        ...
        outgoingHeaders['Authorization'] = `Basic ${Buffer.from(freshdeskApiKey + ':X').toString('base64')}`;
